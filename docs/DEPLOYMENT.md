...
- Managed PostgreSQL database for public sync features (Vercel Postgres, Neon, Supabase, Railway, Render, or another hosted Postgres provider)
...
# Managed Postgres for bracket rooms and watchlist alert sync
DATABASE_URL=postgres://...
# or
POSTGRES_URL=postgres://...

# Optional serverless pool limit
PGPOOL_MAX=3
...
### 4.1. Initialize Managed Postgres

The app auto-creates the required tables the first time `/api/bracket-rooms`, `/api/watchlist-alerts`, or `/api/launch-readiness` runs with a database URL configured. For an explicit migration step, run:

```bash
psql "$DATABASE_URL" -f db/fotpredict_sync_schema.sql
```

The production sync schema stores:

- `fotpredict_bracket_rooms` - tournament challenge groups, room codes, commissioner PIN hashes, and synced entries.
- `fotpredict_watchlist_alert_rooms` - watchlist alert queues, tracked teams, alert settings, and sync codes.

After deployment, open:

```bash
https://your-project.vercel.app/api/launch-readiness
```

`launch_ready` should be `true`. If it is `false`, the app is still running on a local fallback store and should not be treated as a real public launch deployment.
