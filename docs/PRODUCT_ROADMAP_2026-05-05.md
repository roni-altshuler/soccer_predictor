...
| 7 | Scenario Simulator | High | High | Premium users should be able to test title/top-4/relegation and tournament paths from live standings. | Advanced: tournament simulator now supports focus-team, favorable/adverse path, volatility controls, saved local scenario cards, JSON export, and local-first bracket challenges with AI bracket entries, invite-link import, plus commissioner scoring rules across World Cup, UEFA club tournaments, Euros, and Copa America. |
...
4. Add calibration trend history to the new per-league model drilldown.
5. Add licensed odds-provider ingestion and responsible product UX around market-intelligence views.
