# Pitchverse (then "Pitchwise", originally "FotPredict AI") Product Roadmap - May 5, 2026

This roadmap triages the next 10 product ideas by a combined score of user impact, implementation effort, model integrity, and monetization potential. The intent is to build Pitchverse like a startup-quality sports intelligence app: trustworthy data first, then deeper prediction tooling, then premium-grade personalization and market intelligence.
...
| 4 | Personal Watchlist Expansion | High | Low-Medium | Retention improves when users can follow teams, matches, and model picks from natural entry points. | Implemented: match detail pages can add either team to the local watchlist, and the Fan Tracking workspace now has local kickoff reminders, confidence thresholds, browser-notification permission handling, active alert queue, and managed-Postgres alert queue sync by code. Native Web Push delivery still needs production provider wiring. |
...
7. Personal Watchlist Expansion — implemented with local alerts and managed-Postgres server sync; production work is native Web Push delivery.
...
- [x] Replace the development-only sync store with managed Postgres support, schema, and a launch-readiness health check.
...
1. Add native Web Push delivery with VAPID/provider credentials on top of the managed server alert queue.
2. Add authentication/accounts so synced bracket rooms and alert queues can belong to real users instead of room codes.
3. Configure a licensed odds-provider account in production and complete a legal/compliance review of market-intelligence wording.
4. Expand fixture-level what-if controls from one locked fixture to multi-fixture scenario slates.
5. Add account-level export/delete controls and retention policies for public launch privacy readiness.
