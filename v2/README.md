...
Production-SaaS rebuild of Pitchverse (formerly "FotPredict AI", then "Pitchwise"; renamed to Pitchverse in July 2026). Lives in the same repo as v1 (one directory up: [../backend/](../backend/), [../src/](../src/)) so the redesign can land incrementally without a fork. FotMob-class live football tracking layered with a deep predictive-analytics product (match outcomes, scorelines, league/bracket Monte Carlo simulations, season-progression forecasts, AI confidence, dynamic in-play updates).
