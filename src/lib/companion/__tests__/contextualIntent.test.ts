...
describe('deriveAskIntent', () => {
  ...
  it('refuses a finished match unless a minute is supplied', () => {
...
describe('contextualPrompts', () => {
  it('names the real teams for a side that is behind', () => {
    ...
    expect(prompts[0].label).toBe("Can Arsenal come back from 2 down at 60'?")
  ...
  it('always asks from the trailing bench, whichever side that is', () => {
    ...
    expect(prompts[0].label).toBe("Can Chelsea come back from 2 down at 60'?")
